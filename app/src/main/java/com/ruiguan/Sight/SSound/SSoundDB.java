...
public class SSoundDB extends SQLiteOpenHelper {
    ...
    public SSoundDB(Context context) {
        super(context, "SSoundSave.db", null, 1);
    }
    //创建table
    @Override
    public void onCreate(SQLiteDatabase db) {
        String sql = "CREATE TABLE " + "SSoundSave" + " (" + "id"+
                " INTEGER primary key autoincrement, " + "date" + " text, "+ "company" +" text, "+"number"+" text, "
                +"sightNumber" +" text, "+ "sightLenght" +" text, "+ "sightType" +" text, "+ "sightLoad" +" text, "+"soundMax"+" real);";
        db.execSQL(sql);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        String sql = "DROP TABLE IF EXISTS " + "SSoundSave";
        db.execSQL(sql);
        onCreate(db);
    }
    public Cursor select() {
        ...
        Cursor cursor = db
                .query("SSoundSave", null, null, null, null, null, null);
        return cursor;
    }
    //增加操作
    public long insert(String str_date,String str_SSoundMax)
    {
        ...
        cv.put("soundMax", str_SSoundMax);

        long row = db.insert("SSoundSave", null, cv);
        return row;
    }
    //删除操作
    public void delete(int id)
    {
        ...
        db.delete("SSoundSave", where, whereValue);
    }
    //修改操作
    public void update(int id)
    {
        ...
        db.update("SSoundSave", cv, where, whereValue);
    }
