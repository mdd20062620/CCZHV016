...
public class SoundDB extends SQLiteOpenHelper {
    ...
    public SoundDB(Context context) {
        super(context, "SoundSave.db", null, 1);
    }
    //创建table
    @Override
    public void onCreate(SQLiteDatabase db) {
        String sql = "CREATE TABLE " + "SoundSave" + " (" + "id"+
                " INTEGER primary key autoincrement, " + "date" + " text, "+ "company" +" text, "+"number"+" text, "
                +"chacheNumber" +" text, "+ "chacheType" +" text, "+ "chacheGroup" +" text, " +"SoundMax"+" real);";
        db.execSQL(sql);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        String sql = "DROP TABLE IF EXISTS " + "SoundSave";
        db.execSQL(sql);
        onCreate(db);
    }
    public Cursor select() {
        ...
        Cursor cursor = db
                .query("SoundSave", null, null, null, null, null, null);
        return cursor;
    }
    //增加操作
    public long insert(String str_date,String str_SoundMax)
    {
        ...
        cv.put("SoundMax", str_SoundMax);

        long row = db.insert("SoundSave", null, cv);
        return row;
    }
    //删除操作
    public void delete(int id)
    {
        ...
        db.delete("SoundSave", where, whereValue);
    }
    //修改操作
    public void update(int id)
    {
        ...
        db.update("SoundSave", cv, where, whereValue);
    }
