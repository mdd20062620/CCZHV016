...
public class ClassicBluetoothManage {
    ...
    public ClassicBluetoothManage(BluetoothSocket socket) {
    ...
    private void threadConnect(BluetoothDevice bluetoothDevice) throws Exception{
        //final BluetoothSocket socket = bluetoothDevice.createRfcommSocketToServiceRecord(UUID.fromString("00001101-0000-1000-8000-00805F9B34FB"));//经典蓝牙的连接UUID
        ...
        new TaskThread(mContext).setWorkCall(new TaskThread.WorkCallBack() {
            ...
            public void succeed() {
                ...
                setSendThread();
                setListenerThread();
                mContext.registerReceiver(mConnectListener,new IntentFilter(BluetoothDevice.ACTION_ACL_DISCONNECTED));//注册断线广播
            }
                            ...
                            if (mInputBluetooth.available()!=0){
                               do {
                                   ...
                                   // if (temp > 200)
                                      ...
                                      // mIDataCallback.readData(bytes,mConnectedMac);
